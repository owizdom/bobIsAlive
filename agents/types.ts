...
export type { MarketDataset } from "./markets";
...
export type OracleDirection = "bullish" | "bearish" | "neutral";
...
export interface OracleConsensus {
  ...
  preCommitProofs: Record<string, string>;  // agentId → "sha256:<hash>"
  resolvedAt: number;
}

/** Discriminated union of possible agent actions */
export type AgentAction =
  | { type: "analyze_market"; ticker: string }
  | { type: "share_prediction"; prediction: string; ticker?: string }
  | { type: "correlate_markets"; tickers: string[] }
  | { type: "scan_sector"; ticker: string };
...
export interface AutonomousAgentState extends AgentState {
  ...
  currentPrediction?: OraclePrediction;
...
export interface SealedBlob {
  ...
  independenceProof: string; // ed25519:<sig(agentId|explorationEndedAt|contentHash)>:<pubkey>
}
...
export interface AgentCommitment {
  ...
  commitmentHash: string;          // "sha256:<hex>" fallback
