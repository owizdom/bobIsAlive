  ...
  getDb().prepare(`
    INSERT INTO commits
      (agent_id, agent_name, agent_public_key, commitment_hash,
       sealed_blob_hash, committed_at, cycle_start_step, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `).run(
    commitment.agentId, commitment.agentName, commitment.agentPublicKey,
    commitment.commitmentHash, commitment.sealedBlobHash, commitment.committedAt, commitment.cycleStartStep, Date.now()
  );
}
...
export function getRecentPheromones(
  limit = 50
): Array<{
  id: string;
  domain: string;
  content: string;
  confidence: number;
  preCommitRef?: string | null;
  eigendaCommitment?: string | null;
  timestamp: number;
}> {
  const d = getDb();
  ...
  return rows.map((r) => ({
    ...
    preCommitRef: (r.eigenda_commitment as string | null),
    timestamp: r.timestamp as number,
  }));
}

export function getCollectiveMemories(): Array<{
  id: string;
  domain: string;
  synthesis: string;
